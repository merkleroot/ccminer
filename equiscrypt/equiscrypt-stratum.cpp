...
double target_to_diff_equiscrypt(uint32_t* target)
{
...
double equiscrypt_network_diff(struct work *work)
{
	...
	double d = target_to_diff_equiscrypt((uint32_t*)net_target);
	return d;
}
...
bool equiscrypt_stratum_set_target(struct stratum_ctx *sctx, json_t *params)
{
	...
	sctx->next_diff = target_to_diff_equiscrypt((uint32_t*) &target_be);
...
// equiscrypthash stratum protocol is not standard, use client.show_message to pass block height
bool equiscrypt_stratum_show_message(struct stratum_ctx *sctx, json_t *id, json_t *params)
{
	...
	if (val) {
		...
		if (data && strlen(data)) {
			char symbol[32] = { 0 };
			uint32_t height = 0;
			int ss = sscanf(data, "equiscrypthash %s block %u", symbol, &height);
...
void equiscrypt_store_work_solution(struct work* work, uint32_t* hash, void* sol_data)
{
	...
	//work->sharediff[nonce] = target_to_diff_equiscrypt(hash);
}
