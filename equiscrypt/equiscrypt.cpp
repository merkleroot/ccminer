...
// hdr -> header including nonce (212 bytes)
...
#else
	eq_blake2b_update(&state, hdr, 212);
#endif

	expandArray(soln, equiscrypthashSolutionSize, (uint8_t*) &indices, sizeof(indices), collisionBitLength + 1, 1);
