...
__global__ void digit_first(equiscrypt<RB, SM>* eq, u32 nonce)
{
...
__global__ void digit_1(equiscrypt<RB, SM>* eq)
{
...
template <u32 RB, u32 SM, int SSM, typename PACKER, u32 MAXPAIRS, u32 THREADS>
__global__ void digit_2(equiscrypt<RB, SM>* eq)
{
...
template <u32 RB, u32 SM, int SSM, typename PACKER, u32 MAXPAIRS, u32 THREADS>
__global__ void digit_3(equiscrypt<RB, SM>* eq)
{
	__shared__ u16 ht[NRESTS][(SSM - 1)];
...
template <u32 RB, u32 SM, int SSM, typename PACKER, u32 MAXPAIRS, u32 THREADS>
__global__ void digit_4(equiscrypt<RB, SM>* eq)
{
	__shared__ u16 ht[NRESTS][(SSM - 1)];
...
template <u32 RB, u32 SM, int SSM, typename PACKER, u32 MAXPAIRS, u32 THREADS>
__global__ void digit_5(equiscrypt<RB, SM>* eq)
{
	__shared__ u16 ht[NRESTS][(SSM - 1)];
	__shared__ uint4 lastword[NSLOTS];
	__shared__ int ht_len[NRESTS];
...
__global__ void digit_6(equiscrypt<RB, SM>* eq)
{
	__shared__ u16 ht[NRESTS][(SSM - 1)];
...
template <u32 RB, u32 SM, int SSM, typename PACKER, u32 MAXPAIRS>
__global__ void digit_7(equiscrypt<RB, SM>* eq)
{
	__shared__ u16 ht[NRESTS][(SSM - 1)];
...
template <u32 RB, u32 SM, int SSM, typename PACKER, u32 MAXPAIRS>
__global__ void digit_8(equiscrypt<RB, SM>* eq)
{
	__shared__ u16 ht[NRESTS][(SSM - 1)];
	__shared__ u32 lastword[NSLOTS][2];
	__shared__ int ht_len[NRESTS];
...
__global__ void digit_last_wdc(equiscrypt<RB, SM>* eq)
{
...
__host__ eq_cuda_context<RB, SM, SSM, THREADS, PACKER>::eq_cuda_context(int thr_id, int dev_id)
{
	...
	device_id = dev_id;
	solutions = nullptr;
	equiscrypt_mem_sz = sizeof(equiscrypt<RB, SM>);
	throughput = NBLOCKS;
	...
	fn_hashdone hashdonef)
{
	...
	setheader(&blake_ctx, tequiscrypthash_header, tequiscrypthash_header_len, nonce, nonce_len);
