...
extern "C" void equiscrypt_hash(const void* input, void* output, int len)
{
...
// input here is 140 for the header and 1344 for the solution (equiscrypt.cpp)
extern "C" int equiscrypt_verify_sol(void * const hdr, void * const sol)
{
	bool res = equiscrypt_verify((uint8_t*) hdr, (uint8_t*) sol);
...
static void cb_solution(int thr_id, const std::vector<uint32_t>& solutions, size_t cbitlen, const unsigned char *compressed_sol)
{
	...
	if (!compressed_sol) {
		...
		nSolution = std::vector<unsigned char>(68);
		for (size_t i = 0; i < cbitlen; i++)
			nSolution[i] = compressed_sol[i];
	}
	int nsol = valid_sols[thr_id];
	if (nsol < 0) nsol = 0;
	if(nSolution.size() == 68) {
		// todo, only store solution data here...
		le32enc(&data_sols[thr_id][nsol][212], 0x000540fd); // sol sz header
		memcpy(&data_sols[thr_id][nsol][213], nSolution.data(), 68);
...
extern "C" int scanhash_equiscrypthash(int thr_id, struct work *work, uint32_t max_nonce, unsigned long *hashes_done)
{
	...
	if (!init[thr_id]) {
		try {
			...
			size_t memSz = solvers[thr_id]->equiscrypt_mem_sz / (1024*1024);
	...
	do {
		...
		if (valid_sols[thr_id] > 0)
		{
			...
			uint8_t _ALIGN(64) full_data[212+1+68] = { 0 };
			uint8_t* sol_data = &full_data[212];

			soluce_count += valid_sols[thr_id];

			for (int nsol=0; nsol < valid_sols[thr_id]; nsol++)
			{
				memcpy(full_data, endiandata, 212);
				memcpy(sol_data, &data_sols[thr_id][nsol][212], 69);
				equiscrypt_hash(full_data, vhash, 212+1+68);

				if (vhash[7] <= Htarg && fulltest(vhash, ptarget))
				{
					bool valid = equiscrypt_verify_sol(endiandata, &sol_data[3]);
					if (valid && work->valid_nonces < MAX_NONCES) {
						work->valid_nonces++;
						memcpy(work->data, endiandata, 212);
						equiscrypt_store_work_solution(work, vhash, sol_data);
...
// cleanup
void free_equiscrypthash(int thr_id)
{
	if (!init[thr_id])
		return;
